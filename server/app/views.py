...
def download(request):
    if request.method == 'POST':
        ...
        # Construct the output name from the saved PDF name since FS
        # handles name collision
        output_name = (name.split("."))[0] + ".csv"

        final_frame_data = convert_to_csv(settings.MEDIA_ROOT+'\\'+name)
        final_frame_data.to_csv(
            (settings.MEDIA_ROOT + "/" + output_name), index=False)
        output_url = fs.url(output_name)
